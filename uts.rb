...
unless skip_serial
  puts 'serial'
  run "#{membind} bin/uts.icc.serial -f inputs/uts/#{workload}.input", niter
  puts
end
...
if task_type == 'untied' || task_type == 'both'
  ...
  num_threads.each do |n|
    run "OMP_NUM_THREADS=#{n} #{membind} bin/uts.icc.omp-tasks -f inputs/uts/#{workload}.input", niter
  end
  puts
end
...
if task_type == 'tied' || task_type == 'both'
  puts 'tied'
  num_threads.each do |n|
    run "OMP_NUM_THREADS=#{n} #{membind} bin/uts.icc.omp-tasks-tied -f inputs/uts/#{workload}.input", niter
  end
  puts
end
