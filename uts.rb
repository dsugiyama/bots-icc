...
if task_type == 'untied' || task_type == 'both'
  ...
  num_threads.each do |n|
    run "CILK_NWORKERS=#{n} bin/uts.icc.omp-tasks -f inputs/uts/#{workload}.input", niter
  end
  puts
end
...
if task_type == 'tied' || task_type == 'both'
  puts 'tied'
  num_threads.each do |n|
    run "CILK_NWORKERS=#{n} bin/uts.icc.omp-tasks-tied -f inputs/uts/#{workload}.input", niter
  end
  puts
end
